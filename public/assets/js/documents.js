function viewDocument(id, name) {
  ...
  fetch(bp + '/api/documents/' + id + '/content', { cache: 'no-store' })
    ...
    .then(function(data){
      ...
      var content = (data.data && data.data.content) || 'Sin contenido disponible';
...
function _loadStats() {
  ...
  fetch(bp + '/api/documents', { cache: 'no-store' })
    ...
    .then(function(data){
      ...
      var totalChunks = 0;
      var totalSize = 0;
      docs.forEach(function(d){ totalChunks += (d.chunkCount || 0); totalSize += (d.fileSize || 0); });
      var el;
      el = document.getElementById('stat-total-docs');    if (el) el.textContent = totalDocs;
      el = document.getElementById('stat-total-vectors'); if (el) el.textContent = totalChunks;
      el = document.getElementById('stat-total-size');    if (el) el.textContent = typeof formatBytes === 'function' ? formatBytes(totalSize) : '—';
    })
    .catch(function(){});
}
  ...
  fetch(bp + '/api/documents', { cache: 'no-store' })
    ...
    .then(function(data){
      ...
      grid.innerHTML = docs.map(function(doc){
        var ext   = (doc.originalName || doc.filename || '').split('.').pop().toLowerCase();
        var name  = doc.originalName || doc.filename || 'Sin nombre';
        var color = FILE_COLORS[ext] || '#7c3aed';
        var icon  = FILE_ICONS[ext] || DEFAULT_ICON;
        var date  = doc.createdAt ? new Date(doc.createdAt).toLocaleDateString('es', {day:'2-digit',month:'short',year:'numeric'}) : '—';
        var size  = typeof formatBytes === 'function' ? formatBytes(doc.fileSize || 0) : '—';
        var chunks = doc.chunkCount != null ? doc.chunkCount : 0;
        ...
        return '<div class="doc-card card" data-doc-id="' + doc.id + '" style="position:relative;">'
          ...
          + '<button class="btn btn-secondary btn-sm doc-view-btn" style="flex:1;" data-id="' + doc.id + '" data-name="' + _esc(name) + '">Ver contenido</button>'
          + '<button class="btn btn-danger btn-sm doc-del-btn" data-id="' + doc.id + '" data-name="' + _esc(name) + '" aria-label="Eliminar">'
      ...
      grid.querySelectorAll('.doc-view-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          viewDocument(parseInt(this.dataset.id, 10), this.dataset.name);
        });
      });
      grid.querySelectorAll('.doc-del-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          confirmDeleteDocument(parseInt(this.dataset.id, 10), this.dataset.name);
        });
      });
    })
    .catch(function(err){
